...
- 80점 이상, 모든 차원 하한 충족, 즉시 보류 없음: `AI_PASS`(AI 평가 통과)
- 65점 이상 80점 미만 또는 항목별 하한·보류 조건 미충족: `REVISION_REQUIRED`(보완 필요)
- 65점 미만: `REJECTED`
- API·timeout·인증·JSON 오류: `ERROR`

모델이 반환하는 총점과 최종 판정은 신뢰하지 않고 서버가 차원 점수로 다시 계산한다.

2026-07-30부터 `improvement_validity_rubric.json`의 `개선안RB1.6`을 배점·판정 구간·항목별 하한·즉시 보류 규칙의 단일 기준으로 사용한다. 평가 서비스는 65점과 80점을 코드에 별도로 고정하지 않고 Rubric의 `automatic_decisions`를 읽어 판정한다. Rubric 저장 시 0~100점 구간의 중복·누락, 필수 판정, 항목별 하한 적용 여부와 필수 보류 규칙을 검증한다.

여기서 규칙 묶음은 개발 내부적으로는 입력·출력과 판정 조건을 보장하는 “평가 계약”으로 볼 수 있지만, 사용자 화면과 운영 문서에서는 의미가 더 분명한 “타당성 평가 기준 및 판정 규칙”으로 표현한다.
...
- 타당성 평가 모델: Anthropic `claude-haiku-4-5`
- 타당성 점수: 77점
...
2026-07-30 `개선안RB1.6` 정합성 보완 검증:

- 64.99점 반려, 65점·79.99점 보완 필요, 80점 AI 평가 통과 경계값: PASS
- 80점 이상 항목별 하한 미달 차단: PASS
- 100점 즉시 보류 규칙 발생 시 AI 평가 통과 차단: PASS
- Rubric 판정 구간을 변경했을 때 서비스가 변경값을 직접 적용: PASS
- 판정 이름 중복·필수 판정·항목 하한·필수 보류 규칙 저장 검증: PASS
- 타당성·VOC 화면 회귀 테스트: 161 PASS
- 품질 계약 검증기와 보고서·최종 승인 연계 테스트 포함: 189 PASS

2026-07-30 AI 평가 통과 실패 원인 시각화:

- 타당성 평가 결과에서 `점수 부족`, `항목별 하한 미달`, `즉시 보류 규칙`, `VOC·Trace 근거 부족`을 4개 진단 카드로 구분한다.
- 점수 부족은 현재 총점, AI 평가 통과 기준, 부족 점수를 표시한다.
- 항목별 하한 미달은 평가 항목별 현재 점수와 통과 하한을 표시한다.
- `missing_voc_or_trace_evidence`는 일반 즉시 보류 목록에서 제외하고 VOC·Trace 근거 부족 영역에만 표시해 중복 집계를 방지한다.
- VOC·Trace 근거 진단은 서버 보류 규칙, Trace ID·이벤트 존재 여부, 근거 추적성 항목의 하한 미달을 함께 확인한다.
- 각 실패 원인에는 사용자가 다음 보완 또는 RETEST에서 수행할 조치를 함께 표시한다.
- 기존 저장 결과도 화면에서 동적으로 진단하므로 과거 `validity_result.json`을 변경하지 않는다.
- 타당성 화면·통합 회귀 테스트: 192 PASS, Streamlit 실패 상태 렌더링 예외 0건
