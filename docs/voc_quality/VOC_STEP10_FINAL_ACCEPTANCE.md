...
- 동료평가 80점과 교수평가 20점 증적 준비 상태
- 잔여 위험과 운영 권고
- 발표 순서와 사용자 최종 승인 대기 상태
- JSON·Markdown 인수 증적 생성과 다운로드

평가 점수는 자동으로 부여하지 않는다. 각 항목의 증적 준비 상태만 `READY` 또는 `PARTIAL`로 표시한다.
...
두 파일은 자동 게이트, 업무 흐름, 평가 증적 준비 상태, 정량 수치, 잔여 위험과 발표 순서를 기록한다. 사용자 최종 서명은 별도로 남겨야 한다.

## 7. 시연 순서

프로젝트 목적 → 6개 Agent → 수동·일괄 수행 → A2A Trace → 독립 Judge → 개선안 타당성 → 장애·결함 → 연결 재시험·A/B → 품질 보고서 → 배포 판정

각 구간에서는 기능 화면보다 Run ID, Case, Trace, Rubric 버전과 판정 근거를 먼저 설명한다.
...
4. 평가 체크리스트가 자동 점수가 아닌 증적 준비 상태인지 확인한다.
5. 잔여 위험과 시연 순서가 실제 발표에 적절한지 검토한다.
6. `Step 10 인수 증적 생성`으로 JSON·Markdown을 생성한다.
...
- 실제 동료평가·교수평가 양식과 자동 증적 체크리스트 항목이 맞는지 검토한다.
