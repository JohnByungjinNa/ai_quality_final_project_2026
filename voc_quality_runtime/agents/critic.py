...
# Port (default bind): 0.0.0.0:6105
# ================================================
...
class CriticServicer(voc_pb2_grpc.CriticServicer):
    ...
    async def Review(self, request, context):
        ...
        try:
            ...
            final_policy = request.doc if role == "policy" else ""
...
async def serve() -> None:
    ...
    기본값은 "0.0.0.0:6105"입니다 (모든 네트워크 인터페이스의 6105 포트).
    """
    # ============ 엔드포인트 설정 ============
    # 환경변수에서 엔드포인트를 읽어오고, 없으면 기본값을 사용합니다
    bind = os.environ.get("CRITIC_BIND", "0.0.0.0:6105")
