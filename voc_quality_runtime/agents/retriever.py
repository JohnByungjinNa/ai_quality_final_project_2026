...
class RetrieverAgent:
    ...
    def __init__(self):
        ...
        self.summarizer_endpoint = os.environ.get("SUMMARIZER_ENDPOINT", "localhost:6103")

    @staticmethod
    def _normalized_text(value: str) -> str:
        """공백과 문장부호 차이를 무시한 비교 문자열을 만듭니다."""
        return "".join(char for char in (value or "").lower() if char.isalnum())

    @staticmethod
    def _filter_tokens(value: str) -> list[str]:
        """복합 검색 구문을 두 글자 이상의 단어로 분리합니다."""
        return [
            token
    ...
    @classmethod
    def _match_score(cls, line: str, filters: list[str]) -> int:
        """여러 구문에 걸친 단어 일치를 점수화하고 모호한 단일 구문은 제외합니다."""
        normalized_line = cls._normalized_text(line)
        exact_matches = 0
        matched_groups = 0
        matched_tokens = set()

        for value in filters:
            normalized_filter = cls._normalized_text(value)
            tokens = cls._filter_tokens(value)
            if not normalized_filter and not tokens:
                continue
            exact = bool(normalized_filter and normalized_filter in normalized_line)
            token_hits = {token for token in tokens if token in line}
            if exact:
                exact_matches += 1
            if exact or token_hits:
                matched_groups += 1
                matched_tokens.update(token_hits)

        if exact_matches:
            return exact_matches * 100 + matched_groups * 10 + len(matched_tokens)
        if matched_groups >= 2 and len(matched_tokens) >= 2:
            return matched_groups * 10 + len(matched_tokens)
        return 0
    ...
    async def run(self, csv_path: str, filters: list[str], max_items: int) -> list[str]:
        ...
        """
        # 장애 진단 전용 지연 주입. 기본값은 0이며 운영 동작에는 영향이 없습니다.
        # 예: A2A_FAULT_RETRIEVER_DELAY_MS=2000
        try:
            delay_ms = max(0, min(int(os.environ.get("A2A_FAULT_RETRIEVER_DELAY_MS", "0")), 300000))
        except ValueError:
            delay_ms = 0
        if delay_ms:
        ...
        with open(csv_path, "r", encoding="utf-8") as fp:
            ...
            for row in csv_reader:
                ...
                score = self._match_score(line, filters) if use_filter else 1
                if score > 0:
                    # 원본 행을 공백으로 결합하여 결과에 추가합니다 (소문자 변환 전 원본 사용)
                    scored_results.append((score, len(scored_results), " ".join(row)))

                    # ============ 최대 개수 도달 시 조기 종료 ============
                    # max_items 개수에 도달하면 더 이상 검색하지 않고 반복문을 종료합니다
                    # 전체 후보를 점수화한 후 관련도 순으로 max_items만 반환합니다.

        # ============ 결과 반환 ============
        scored_results.sort(key=lambda item: (-item[0], item[1]))
        return [item[2] for item in scored_results[:max_items]]
...
async def serve():
    ...
    기본값은 "0.0.0.0:6102"입니다 (모든 네트워크 인터페이스의 6102 포트).
    """
    # ============ 엔드포인트 설정 ============
    # 환경변수에서 엔드포인트를 읽어오고, 없으면 기본값을 사용합니다
    endpoint = os.environ.get("RETRIEVER_ENDPOINT", "0.0.0.0:6102")
