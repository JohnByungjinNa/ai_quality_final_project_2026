...
from utils.a2a_audit import audited_rpc, extract_keywords, new_trace_id, record_event

SAFE_NO_DATA_MESSAGE = (
...
class SummarizerAgent:
    ...
    def __init__(self):
        ...
        self.llm = OpenAIChat()
        # 다음 에이전트 엔드포인트 설정
        self.evaluator_endpoint = os.environ.get("EVALUATOR_ENDPOINT", "localhost:6104")
        # run_pipeline에서 사용하는 에이전트 엔드포인트 설정
        self.retriever_endpoint = os.environ.get("RETRIEVER_ENDPOINT", "localhost:6102")
        self.critic_endpoint = os.environ.get("CRITIC_ENDPOINT", "localhost:6105")
        self.improver_endpoint = os.environ.get("IMPROVER_ENDPOINT", "localhost:6106")
...
"""
    ...
    async def run_pipeline(
        ...
        """
        trace = []
        audit_trace_id = trace_id or new_trace_id()
        trace.append(f"audit_trace_id={audit_trace_id}")
        
        # ============ 1단계: Retriever 호출 ============
        try:
            async with grpc.aio.insecure_channel(self.retriever_endpoint) as ch:
                stub = voc_pb2_grpc.RetrieverStub(ch)
                rres = await audited_rpc(stub.Retrieve(
                    voc_pb2.RetrieveReq(
                        csv_path=csv_path,
                        filters=filters,
                        max_items=max_items,
                    ...
                    timeout=timeout
                ), trace_id=audit_trace_id, source="Summarizer", target="Retriever",
                   operation="Retrieve", input_data={"filters": filters, "max_items": max_items})
        except grpc.aio.AioRpcError as exc:
            code = exc.code().name if exc.code() else "UNKNOWN"
            details = (exc.details() or "세부 오류 없음").strip()
            raise RuntimeError(
                f"검색 불가: Retriever 호출 실패 [{code}] {details}"
            ) from exc
        ...
        if not texts:
            return {
                "summary": SAFE_NO_DATA_MESSAGE,
                "policy": "",
                "eval_json": "{}",
                "summary_critic_json": "{}",
                "trace": "; ".join(trace + ["no_related_data"]),
                "ok": False,
        ...
        record_event(trace_id=audit_trace_id, source="Retriever", target="Summarizer",
                     operation="VOCDataTransfer", status="success", duration_ms=0,
                     keywords=extract_keywords(texts), item_count=len(texts))
        ...
        async with grpc.aio.insecure_channel(self.evaluator_endpoint) as ch:
            ...
            eres = await audited_rpc(stub.Evaluate(
                voc_pb2.EvaluateReq(
                    task=task,
                    candidates=candidates
                ),
                timeout=timeout
            ), trace_id=audit_trace_id, source="Summarizer", target="Evaluator",
               operation="Evaluate", input_data=candidates)
        
        winner_key = eres.winner or sorted(candidates.keys())[0]
        if winner_key not in candidates and f"S{winner_key}" in candidates:
            winner_key = f"S{winner_key}"
        if winner_key not in candidates:
            winner_key = sorted(candidates.keys())[0]
        ...
        async with grpc.aio.insecure_channel(self.critic_endpoint) as ch:
            ...
            cres = await audited_rpc(stub.Review(
                voc_pb2.ReviewReq(
                    doc=summary,
                    role="summary"
                ),
                timeout=timeout
            ), trace_id=audit_trace_id, source="Summarizer", target="Critic",
               operation="ReviewSummary", input_data=summary)
        ...
        if task in ("policy", "both"):
            async with grpc.aio.insecure_channel(self.improver_endpoint) as ch:
                ...
                pres = await audited_rpc(stub.Improve(
                    voc_pb2.PolicyReq(summary=summary), timeout=timeout
                ), trace_id=audit_trace_id, source="Summarizer", target="Improver",
                   operation="ImprovePolicy", input_data=summary)
            policy = pres.policy or ""

            async with grpc.aio.insecure_channel(self.critic_endpoint) as ch:
                stub = voc_pb2_grpc.CriticStub(ch)
                pcres = await audited_rpc(stub.Review(
                    voc_pb2.ReviewReq(doc=policy, role="policy"), timeout=timeout
                ), trace_id=audit_trace_id, source="Summarizer", target="Critic",
                   operation="ReviewPolicy", input_data=policy)

            if pcres.need_refine and pcres.edits:
                async with grpc.aio.insecure_channel(self.improver_endpoint) as ch:
                    stub = voc_pb2_grpc.ImproverStub(ch)
                    refined = await audited_rpc(stub.Refine(
                        voc_pb2.RefinePolicyReq(
                            policy=policy,
                            edits_json=json.dumps({"edits": list(pcres.edits)}, ensure_ascii=False),
                        ), timeout=timeout
                    ), trace_id=audit_trace_id, source="Summarizer", target="Improver",
                       operation="RefinePolicy", input_data=list(pcres.edits))
                policy = refined.policy or policy
...
async def serve():
    ...
    기본값은 "0.0.0.0:6103"입니다 (모든 네트워크 인터페이스의 6103 포트).
    """
    # ============ 엔드포인트 설정 ============
    # 환경변수에서 엔드포인트를 읽어오고, 없으면 기본값을 사용합니다
    endpoint = os.environ.get("SUMMARIZER_ENDPOINT", "0.0.0.0:6103")
