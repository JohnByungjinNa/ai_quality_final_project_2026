...
from utils.a2a_audit import audited_rpc, new_trace_id

# ============ gRPC 에이전트 엔드포인트 설정 ============
# 각 에이전트 서비스의 네트워크 주소를 환경변수에서 읽어옵니다
# 환경변수가 없으면 기본값(localhost)을 사용합니다
# 각 에이전트는 독립적인 포트에서 실행됩니다
INTERPRETER_ENDPOINT = os.environ.get("INTERPRETER_ENDPOINT", "localhost:6101")  # 자연어 질의 해석 서비스
RETRIEVER_ENDPOINT   = os.environ.get("RETRIEVER_ENDPOINT",   "localhost:6102")  # VOC 데이터 검색 서비스
SUMMARIZER_ENDPOINT  = os.environ.get("SUMMARIZER_ENDPOINT",  "localhost:6103")  # 요약 생성 서비스
EVALUATOR_ENDPOINT   = os.environ.get("EVALUATOR_ENDPOINT",   "localhost:6104")  # 요약 평가 서비스
CRITIC_ENDPOINT      = os.environ.get("CRITIC_ENDPOINT",      "localhost:6105")  # 요약/정책 비평 서비스
IMPROVER_ENDPOINT    = os.environ.get("IMPROVER_ENDPOINT",    "localhost:6106")  # 정책 개선안 생성 서비스
...
class VOCGRPCRuntime:
    ...
    async def run_with_question(
        ...
        async with grpc.aio.insecure_channel(INTERPRETER_ENDPOINT) as ch:
            ...
            res = await audited_rpc(stub.ParseQuestion(
            ...
            ), trace_id=audit_trace_id, source="Orchestrator", target="Interpreter",
               operation="ParseQuestion", input_data=question)
        ...
        async with grpc.aio.insecure_channel(SUMMARIZER_ENDPOINT) as ch:
            ...
            sres = await audited_rpc(stub.RunPipeline(
                ...
                timeout=timeout
            ), trace_id=audit_trace_id, source="Orchestrator", target="Summarizer",
               operation="IntentDataTransfer", input_data=intent)
        ...
        return {
            ...
            "message": (
                ...
                if sres.ok else (sres.summary or "파이프라인이 결과 없이 종료되었습니다.")
            ),
        }
